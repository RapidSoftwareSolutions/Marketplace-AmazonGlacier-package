...
$app->post('/api/AmazonGlacier/getJobs', function ($request, $response, $args) {
    ...
    if (!empty($post_data['args']['completed'])) {
        $body['completed'] = $post_data['args']['completed'];
    }
    ...
    if (!empty($post_data['args']['statuscode'])) {
        $body['statuscode'] = $post_data['args']['statuscode'];
    }

    try {
        $res = $client->listJobs($body)->toArray();
